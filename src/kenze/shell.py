...
COMMANDS = {
    ...
    "validate": ("LOOK", "check the file against a schema json (--scaffold writes one)"),
    ...
    "set":    ("SHELL", "session settings: memory / threads / skip-bad / strict-csv / temp / disk-check"),
...
CMD_GUIDE = {
    ...
    "set":     ("a setting", "set memory 4GB", True,
                ["memory", "threads", "temp", "disk-check", "skip-bad", "strict-csv"]),
    "cd":      ("a folder", "cd outputs", True, []),
    "validate": ("a schema file, or --scaffold to write one",
